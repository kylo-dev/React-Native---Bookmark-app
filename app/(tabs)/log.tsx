...
export default function LogScreen() {
    ...
    const fetchQuotes = async () => {
        try {
            const data = await apiQuotes.getLikedQuotes();
            setQuotes(data || []);
        } catch (e) {
                ...
                <View style={styles.listContainer}>
                    {quotes.length === 0 ? (
                        <View style={styles.emptyStateContainer}>
                            ...
                            <Text style={styles.emptyStateDesc}>책을 읽고 문장을 남기면 이곳에 활동이 기록됩니다.</Text>
                        </View>
                    ) : (
                        quotes.map((quote) => {
                            ...
                            return (
                                <TouchableOpacity
                                    ...
                                    onPress={() =>
                                        router.push({
                                            pathname: '/book/quote',
                                            params: { bookId: quote.book_id.toString(), quoteId: quote.id.toString() },
                                        })
                                    }
                                >
